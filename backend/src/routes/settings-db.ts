...
import { configDBService } from '../services/config-db';
 ...
 */
router.get('/', requireAuthDB, async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.userId) {
      res.status(401).json({
        success: false,
    ...
    const settings = await configDBService.getCurrentSettings(req.userId);
...
/**
 * POST /api/settings
 * Save user settings (with encryption)
 */
router.post('/', requireAuthDB, async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.userId) {
      res.status(401).json({
        success: false,
    ...
    await configDBService.saveUserSettings(req.userId, settings);

    // Return masked settings
    const maskedSettings = await configDBService.getCurrentSettings(req.userId);
 ...
 * Test connection to an integration with provided credentials
 */
router.post('/test/:integration', requireAuthDB, async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.userId) {
      res.status(401).json({
        success: false,
